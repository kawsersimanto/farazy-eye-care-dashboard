...
export const medicineCategoryApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    getMedicineCategorys: builder.query<ApiResponse<IMedicineCategory[]>, void>({
      query: () => "/medicine-category",
      providesTags: ["medicine-category"],
    }),
    getMedicineCategoryById: builder.query<ApiResponse<IMedicineCategory>, string>({
      query: (id) => `/medicine-category/${id}`,
      providesTags: ["medicine-category"],
    }),
    createMedicineCategory: builder.mutation<IMedicineCategory, Partial<IMedicineCategory>>({
      query: (body) => ({ url: "/medicine-category", method: "POST", body }),
      invalidatesTags: ["medicine-category"],
    }),
    updateMedicineCategory: builder.mutation<IMedicineCategory, Partial<IMedicineCategory> & { id: string }>(
      {
        query: ({ id, ...body }) => ({
    ...
    deleteMedicineCategory: builder.mutation<{ success: boolean; id: string }, string>({
